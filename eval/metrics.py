...
from .mle import get_evaluator
from.match_score import eval_match_score
from.bio_match_score import eval_bio_match_score
from.mle_wtext import evaluate_mle_with_text
...
class TabMetrics(object):
    ...
    def evaluate_mle_wtext(self, syn_data):
        real_data = pd.read_csv(self.real_data_path)
        real_data = real_data.apply(lambda x: x.str.strip() if x.dtype == "object" else x)
        test_data = pd.read_csv(self.test_data_path)
        test_data = test_data.apply(lambda x: x.str.strip() if x.dtype == "object" else x)

        dataset_name = self.info.get("name") if isinstance(self.info, dict) else None
        out_metrics = evaluate_mle_with_text(
            real_data, syn_data, test_data,
            do_ttr=False, dataset_name=dataset_name,
        )
        out_extras = {}
        return out_metrics, out_extras


    def evaluate_match_score(self, syn_data):
        out_metrics, out = eval_match_score(syn_data)
        out_extras = {}
        return out_metrics, out_extras

    def evaluate_bio_match_score(self, syn_data):
        out_metrics, out = eval_bio_match_score(syn_data)
        out_extras = {}
        return out_metrics, out_extras
    ...
    def evaluate_density(self, syn_data):
        ...
        for col in real_data.columns:
            target_type = real_data[col].dtype
            try:
                syn_data[col] = syn_data[col].astype(target_type)
            except Exception as e:
                print(f"column {col} fails to convert to {e}")
    ...
    def evaluate_mle(self, syn_data):
        ...
        # drop na
        test = test.dropna(axis=0).to_numpy()
        val = pd.read_csv(self.val_data_path) if self.val_data_path else None
        val = val.apply(lambda x: x.str.strip() if x.dtype == "object" else x).to_numpy() if self.val_data_path else None
...
def reorder(real_data, syn_data, info):
    ...
    if task_type == 'regression':
        num_col_idx += target_col_idx
    elif task_type == 'consistence_check':
        pass
    elif task_type == 'mulclass':
        cat_col_idx += target_col_idx
    else:
        cat_col_idx += target_col_idx
